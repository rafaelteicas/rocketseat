...
export function Quiz() {
  ...
  async function shakeAnimation() {
    ...
    shake.value = withSequence(withTiming(3, { 
      duration: 400,
 ...
 const scrollHandler = useAnimatedScrollHandler({
    ...
    return {
      transform: [{
        translateX: interpolate(
          shake.value,
          [0, 0.5, 1, 1.5, 2, 2.5, 3], [0, -15, 0, 15, 0, -15, 0]
        )
      }]
    }
  })
  ...
  const fixedProgressBarStyles = useAnimatedStyle(() => {
    return {
      ...
      width: '110%',
      left: '-5%',
      opacity: interpolate(scrollInY.value, [50, 90], [0, 1], Extrapolate.CLAMP),
      transform: [{
        translateY: interpolate(
          scrollInY.value,
          [50, 100], [-40, 0], Extrapolate.CLAMP
        )
      }],
      zIndex: 1
    }
  ...
  const headerStyles = useAnimatedStyle(() => {
    return {
      opacity: interpolate(scrollInY.value, [60, 90], [1, 0], Extrapolate.CLAMP)
    }
  })
  ...
  const onPan = Gesture
    ...
    .onUpdate(e => {
      const moveToLeft = e.translationX < 0;
      if (moveToLeft) {
        cardPosition.value = e.translationX
      }
    }) 
    .onEnd((event) => {
